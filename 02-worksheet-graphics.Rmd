...
The above demonstrates the power of `theme()` but the thought of copying this code for each visual you create is not very appealing. A straightforward way of re-using themes is to assign them to an object, then you can add them to the end of your `ggplot` code like any other theme. The below code chunk uses the same theme as above, but assigns it to an object called `my_theme` and then adds it to a new density plot using some of the skills we learnt earlier. Notice that now, when we want to make small changes, like removing the legend, we only have a very small theme section following `my_theme`. Carefully written themes have blanket use for all your graphs!
