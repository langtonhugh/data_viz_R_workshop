...
burglary_df <-  read_csv(file = "https://raw.githubusercontent.com/langtonhugh/data_viz_workshop/master/data/gmp_2017.csv")
```
...
>>>>>>> 38bda821f6f7bad8414a2dc295e9953cfd4f1965
...
So far, we've only explored scatter plots. Let's take a quick look at some other geometries, and with it, some of the other aesthetics available. This list is not exhaustive! But it will cover some key points to help you get started.

### Histogram

Whilst creating a scatter plot needs two existing variables, mapped to the x and y axis, some `ggplot` geometries perform calculations in the background. [Histograms](https://www.mathsisfun.com/data/histograms.html), for instance, visualise the distribution of a variable by creating bins and counting the number of values in each one. The `geom_histogram()` geometry will do this automatically, and thus you only need to specify the x aesthetic. The y axis (count) is being generated for us. The appearance of histograms, and thus the conclusions drawn from the visual, is sensitive to the number of bins, so it is worth investigating several options before settling on a number. Let's take a look at _IMDscore_, the overall score indicating the level of deprivation in each neighbourhood.
=======
Note that in making these tweaks to the visual apparence of geometries, **we have not specified them as aesthetics**. This is because we are making changes to the general visual appearance of the point geometry, rather changing the size or alpha according to a variable in our data frame. Anything specified within `aes()` should be a variable in your data!

What would you expect to happen if you placed `size` inside `aes()` in relation to a variable, such as `size = LAname`? Try it out and see what happens. Again, don't be scared of errors or weird outputs - it's a good way to learn what does what.
...
ggplot(data = burglary_df) +
  geom_histogram(mapping = aes(x = IMDscore), bins = 60)
```
...
<<<<<<< HEAD
Another way you can explore such things is through a facet.
...
Different types of geometry: density with facet
>>>>>>> 38bda821f6f7bad8414a2dc295e9953cfd4f1965
...
<<<<<<< HEAD
How might we use a facet to explore the distribution of IMD deciles by Local Authority, as an alternative to the `position = "dodge"` argument used earlier?

### Lines

```{r, warning=F, message=F}
monthly_df <- read_csv("https://github.com/langtonhugh/data_viz_workshop/raw/master/data/gmp_monthly_2017.csv")
```

```{r}
ggplot(data = monthly_df, aes(x = Month, y = n, group = crime_type, colour = crime_type)) +
  geom_line()
```
...
You might have noticed that [I am a fan of black themes](https://theconversation.com/even-the-most-beautiful-maps-can-be-misleading-126474) for data visualisations. Why? Because they are awesome. With that in mind, let's try and make our scatter plot using a black theme. Each line has been commented to explain what each bit of code is doing.

```{r, out.width="900px"}
p1 + theme_minimal() +
      theme(axis.ticks   = element_blank(),
            plot.title   = element_text(colour = "white", size = 10, family = "mono"),
            plot.caption = element_text(colour = "white", size = 6 , family = "mono", hjust = 1.8),
            axis.title   = element_text(colour = "white", size = 8 , family = "mono"),
            axis.text    = element_text(colour = "white", size = 5 , family = "mono"),
            legend.title = element_blank(),
            legend.text  = element_text(colour = "white", size = 8 , family = "mono"),
            panel.grid.minor = element_blank(),
            panel.grid.major = element_blank(),
            panel.background = element_rect(fill = "grey12", colour = "grey12"),
            strip.text = element_text(colour = "white", size = 6, family = "mono"),
...
The above demonstrates the power of `theme()` but the thought of copying this code for each visual you create is not very appealing. A straightforward way of re-using themes is to assign them to an object, then you can add them to the end of your `ggplot` code like any other theme. The below code chunk uses a the theme as above, but assigns it to an object called `my_theme` and then adds it to a new density plot using some of the skills we learnt earlier. Notice that now, when we want to make small changes, like removing the legend, we only have a very small theme section following `my_theme`. Carefully written themes have blanket use for all your graphs!
...
ggplot(data = burglary_df) +
  ...
  labs(title = "Density of neighbourhood deprivation by Local Authority",
       x = "IMD score") +
...
The folks over at [Trafford Data Lab](https://www.trafforddatalab.io/) wrote a useful [blog](https://medium.com/@traffordDataLab/developing-a-data-visualisation-style-cd24f88fa59) about how to develop your own data visualisation style, and save themes as functions to be re-used.

If you want to expand on the default themes, without these extra tweaks, there are also a number of [packages](https://www.shanelynn.ie/themes-and-colours-for-r-ggplots-with-ggthemr/) which contain more off-the-shelf themes too.

## Arranging graphics

Once you've created a handful of `ggplot` graphics, you may want to arrange them on a page or a poster. So far, we've just been plotting them directly one-by-one. There are a number of ways 

```{r, echo=FALSE, fig.align= "center", fig.width="150px", fig.cap="Source: [cowplot](https://cran.r-project.org/web/packages/cowplot/vignettes/introduction.html)"}
knitr::include_graphics("img/cow.png")
```

## Saving graphics

This is how to save a ggplot object.
